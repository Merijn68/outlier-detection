...
This deep learning foundation model enables advanced time series anomaly detection with a simple interface. The model assigns anomaly scores that can be thresholded for binary anomaly classification.
The TSPulse method scored considerably worse - as the method was not able to pinpoint the anomalies on the exact date of the occurance. Causing false positives surrounding the actual anomalies.
...
Statistical methods alone achieve limited coverage and/or precision, but windowed methods like LOF and LSTM Autoencoders achieve near perfect detection on injected spike anomalies.
---
